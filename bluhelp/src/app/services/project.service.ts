...
export class ProjectService {
  ...
  getProject(id: number): Observable<ProjectView>{
    return this.http.get<ProjectView>(this.endpoint + id);
  }
  getProjectsWithProgress(progress: number): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'progress/' + progress);
  }
  getProjectsWithCategory(category: number): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'category/id/' + category);
  }
  getProjectsWithDistrict(district: string): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'address/district' + district);
  }
  getProjectsWithCreator(name: string, surname: string): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'creator/name/surname' + name + surname);
  }
  getProjectsWithTitle(title: string): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'title/' + title);
  }
  getTop4Projects(): Observable<Array<ProjectGet>>{
    return this.http.get<Array<ProjectGet>>(this.endpoint + 'default');
  }
